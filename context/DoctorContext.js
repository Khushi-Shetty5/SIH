...
export function DoctorProvider({ children }) {
  ...
  const [patients, setPatients] = React.useState([
    { id: "p1", name: "John Doe", gender: "M", age: 34, lastVisit: "2025-08-12" },
    { id: "p2", name: "Anita Kumari", gender: "F", age: 29, lastVisit: "2025-07-26" },
  ]);

  const [reports, setReports] = React.useState([
    { id: "r1", patientId: "p1", title: "CBC.pdf", type: "pdf", status: "new", createdAt: Date.now() - 3 * 24 * 60 * 60 * 1000 },
    { id: "r2", patientId: "p2", title: "X-Ray.png", type: "image", status: "reviewed", createdAt: Date.now() - 5 * 24 * 60 * 60 * 1000 },
  ]);

  const [appointments, setAppointments] = React.useState([
    { id: "a1", patientId: "p1", when: new Date().setHours(10, 0, 0, 0), status: "scheduled" },
    { id: "a2", patientId: "p2", when: new Date().setHours(10, 30, 0, 0), status: "scheduled" },
  ]);
  ...
  const addReport = ({ patientId, title, type = "note" }) => {
    const id = `r${Date.now()}`;
    setReports((prev) => [{ id, patientId, title, type, status: "new", createdAt: Date.now() }, ...prev]);
  };
